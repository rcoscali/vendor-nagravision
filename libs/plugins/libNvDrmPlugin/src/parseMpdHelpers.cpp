...
findCencElement(xmlNodePtr cur)
{
  ...
  while (contentProtectionElement)
      ...
      if (schemeIdUriAttr)
	{
	  ...
	  if (String8(value) == NvDrmPlugin::cencUuid)
	    {
...
findDrmScheme(xmlNodePtr cur)
{
  ...
  while (contentProtectionElement)
      ...
      if (schemeIdUriAttr)
	{
	  ...
	  if (String8(value) == NvDrmPlugin::drmSchemesFind("MSPlayReady") ||
	      String8(value) == NvDrmPlugin::drmSchemesFind("Dummy1"))
	    return true;
	}
...
findSupportedMediaUri(xmlNodePtr cur)
{
  ...
  /*
   * Representation elt / mimeType attr => mimeTypes
   */
  ...
  /*
   * AdaptationSet elt / mimeType attr => mimeTypes
   */
  ...
  /*
   * SegmentTemplate elt / media attr => URLs
   */
